import React from "react";
import styles from "../About/About.module.css"
import  pic from "../About/team/waqas.png"
import  arslanpic from "../About/team/arslan.png"
const About = () => {
    return (
        <div className={styles.about_section1}>
            <div className={styles.about_section}>
                <h1 className={styles.nametext} >About Us</h1>
                <i>Our Team is very active and motivated with own works</i>
                <i>We will try to become a good E-commerece Website</i>
            </div>
            <h2 className={styles.teamtxt}><u>Our Team</u></h2>
            <div className={styles.first_div}>
                <div className={styles.container}>
                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6kTvOazDqSxK2ManXX77UNIniLMoQXlZ8iA&usqp=CAU" className={styles.image} />
                    <div className={styles.overlay}>
                        <div className={styles.text}>
                            <h1>Developer</h1>
                            <hr className={styles.line} />
                            <p>i love to help with our team and i am highly motivated.</p></div>
                    </div>
                    <h1 className={styles.hamadtxt}>Hammad Ali</h1>
                </div>
                {/* card 2 */}

                <div className={styles.container}>
                    <img src={pic} className={styles.image} />
                    <div className={styles.overlay}>
                        <div className={styles.text}>
                            <h2>Developer</h2>
                            <hr className={styles.line} />
                            <p>I love helping others and pushing them to do their best!</p>
                        </div>
                    </div>
                    <h1 className={styles.hamadtxt}>Waqas Khan</h1>
                </div>
            </div>

            {/* card3  */}
            <div className={styles.first_div}>
                <div className={styles.container}>
                    <img src={arslanpic} className={styles.image} />
                    <div className={styles.overlay}>
                        <div className={styles.text}>
                            <h1>Co-Founder</h1>
                            <hr className={styles.line} />
                            <p>I love helping others and pushing them to do their best! </p></div>
                    </div>
                    <h1 className={styles.hamadtxt}>Arslan Ahmed</h1>
                </div>

                {/* card 4  */}

                <div className={styles.container}>
                    <img src="data:image/jpeg;base64," className={styles.image} />
                    <div className={styles.overlay}>
                        <div className={styles.text}>
                            <h1>Co-Founder</h1>
                            <hr class={styles.line}/>
                            <p>I love helping others and pushing them to do their best!</p>
                        </div>
                    </div>
                    <h1 className={styles.hamadtxt}>Sheraz khan</h1>
                </div>

            </div>

        </div>

    )
}
export default About;